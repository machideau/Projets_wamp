<?php
    ...
    if(isset($_POST['submit'])){ // if user click the submit btn
        ...
        // declaring input
        // echo $name, $email, $image_rename, $ran_id;
        ...
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){// checking if email is valid
            ...
            if(mysqli_num_rows($select) > 0){
                ...
                if($password != $cpassword){
                    ...
                    $insert = mysqli_query($conn, "INSERT INTO `user_form`(`unique_id`, `name`, `email`, `password`, `img`, `status`) 
                    VALUES ('$ran_id','$name','$email','$password','$image_rename','$status')") or die("erreur");
